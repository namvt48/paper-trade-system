...
@pytest.mark.asyncio
async def test_repeated_mismatch_logs_transition_and_periodic_summary(tmp_path, monkeypatch):
    db = Database(str(tmp_path / "positions.db"))
    await db.init()
    await db.register_alpha("a")
    await db.create_position("p1", "a", "s1", "BTCUSDT", "LONG", 100, 1,
    ...
    now = 0.0
    warning = Mock()
    monkeypatch.setattr("app.position_ownership.logger.warning", warning)
    monitor = PositionOwnershipMonitor(
        db, redis, redis, grace_sec=0, clock=lambda: now, summary_interval_sec=300
    )
...
@pytest.mark.asyncio
async def test_recovery_logs_once(tmp_path, monkeypatch):
    db = Database(str(tmp_path / "positions.db"))
    await db.init()
    await db.register_alpha("a")
    await db.create_position("p1", "a", "s1", "BTCUSDT", "LONG", 100, 1,
    ...
    mds = fakeredis.aioredis.FakeRedis(decode_responses=True)
    info = Mock()
    monkeypatch.setattr("app.position_ownership.logger.info", info)
    monitor = PositionOwnershipMonitor(db, paper, mds, grace_sec=0)
    ...
    info.assert_called_once_with("[POSITION-OWNERSHIP] recovered")
    await db.close()
