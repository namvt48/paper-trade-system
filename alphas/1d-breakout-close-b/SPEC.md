...
```
long_when : (ts_zscore(high, 90) > 1.5)
short_when: (ts_zscore(quote_volume, 15) < -1.5)
```

## Rule

```
long  WHERE (ts_zscore(high, 90) > 1.5)
short WHERE (ts_zscore(quote_volume, 15) < -1.5)
```

## Logic ra/vào lệnh

- **VÀO LONG:** coin thỏa `(ts_zscore(high, 90) > 1.5)`.
- **VÀO SHORT:** coin thỏa `(ts_zscore(quote_volume, 15) < -1.5)`.
...
Fields: `high`, `quote_volume` — see [data](reference/data.md).
