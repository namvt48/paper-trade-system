...
```
long_when : rank(decay_linear(ts_zscore(close_t180, 2880), 240)) > 0.9
short_when: rank(decay_linear(ts_zscore(close_t180, 2880), 240)) < 0.1
```

## Signal (score per symbol, then ranked cross-sectionally)

```
score = decay_linear(ts_zscore(close_t180, 2880), 240)
```
