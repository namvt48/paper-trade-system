...
```
long_when : rank(cs_zscore(decay_linear(ts_zscore(close_t180, 2160), 120)) + cs_zscore(ts_mean(returns_t180, 480))) > 0.9
short_when: rank(cs_zscore(decay_linear(ts_zscore(close_t180, 2160), 120)) + cs_zscore(ts_mean(returns_t180, 480))) < 0.1
```

## Signal (score per symbol, then ranked cross-sectionally)

```
score = cs_zscore(decay_linear(ts_zscore(close_t180, 2160), 120)) + cs_zscore(ts_mean(returns_t180, 480))
```
