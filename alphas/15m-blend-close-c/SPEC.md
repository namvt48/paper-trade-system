...
```
long_when : rank(cs_zscore(ts_zscore(close_t180, 5760)) + cs_zscore(ts_mean(returns_t180, 1920))) > 0.9
short_when: rank(cs_zscore(ts_zscore(close_t180, 5760)) + cs_zscore(ts_mean(returns_t180, 1920))) < 0.1
```

## Signal (score per symbol, then ranked cross-sectionally)

```
score = cs_zscore(ts_zscore(close_t180, 5760)) + cs_zscore(ts_mean(returns_t180, 1920))
```
