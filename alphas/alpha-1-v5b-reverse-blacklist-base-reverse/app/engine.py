...
class Alpha1V5bReverseEngine(BaseEngine):
    """Paper-trade engine for Alpha-1 V5b Reverse.

    Identical to Alpha1V5bEngine except entry direction is flipped:
    - Long signal (trend=True)  → enter SHORT; SL = entry + tp_dist, TP = entry - trail_dist
    - Short signal (trend=False) → enter LONG;  SL = entry - tp_dist, TP = entry + trail_dist

    All other mechanics (trailing stop, Kelly sizing, CUT/REV/TIME exits) are unchanged.
    """
    ...
    def _open_new_position(
        ...
        if not self.can_open_new_trades():
            return
        # Reverse: Long signal → SHORT entry, Short signal → LONG entry.
        # TP/SL are the mirror of v5b: TP = original SL, SL = original TP.
        side       = "SHORT" if trend else "LONG"
        trail_dist = self._adaptive_trail(acol, atr)
        tp_dist    = trail_dist * settings.TP_RATIO
        entry      = close

        if side == "SHORT":  # Long signal reversed to Short
            sl = entry + tp_dist    # was TP of v5b Long
            tp = entry - trail_dist  # was SL of v5b Long
            hse, lse = entry, entry
        else:                # Short signal reversed to Long
            sl = entry - tp_dist    # was TP of v5b Short
            tp = entry + trail_dist  # was SL of v5b Short
            hse, lse = entry, entry
