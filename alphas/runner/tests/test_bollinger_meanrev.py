"""Correctness coverage for the bollinger_meanrev runner strategy.
...
Semantics:
  - Zones computed from the closed 1d prefix: Z < -2 -> LONG (price below
    lower band), Z > 0 -> SHORT (price back above MA), else BETWEEN.
  - No position + LONG/SHORT  -> open that side.
  - No position + BETWEEN     -> stay flat.
  - Same-side zone            -> hold.
  - Opposite extreme zone     -> close old position and open the new side
    (flip, reason ZONE_FLIP).
  - BETWEEN with a LONG       -> hold (giu lenh).
  - BETWEEN with a SHORT      -> close to cash (cash lenh, ZONE_BETWEEN_CASH).
...
def test_zone_long_when_z_below_minus_two() -> None:
    closes = tuple([100.0] * 19 + [50.0])
    assert _zone_from_closes(closes, period=20) == ZONE_LONG


def test_zone_short_when_z_positive() -> None:
    closes = tuple([100.0] * 19 + [150.0])
    assert _zone_from_closes(closes, period=20) == ZONE_SHORT
...
def strategy_with_position(ctx, side: str) -> BollingerMeanRevRunnerStrategy:
...
@pytest.mark.asyncio
async def test_no_position_long_zone_opens_long() -> None:
    closes = tuple([100.0] * 38 + [50.0, 52.0])  # [...,50] dip -> LONG, fill at 52
...
@pytest.mark.asyncio
async def test_no_position_short_zone_opens_short() -> None:
    closes = tuple([100.0] * 38 + [150.0, 148.0])  # [...,150] spike -> SHORT
    strategy = _strategy(ctx_for(closes, AsyncMock(return_value={"ok": True})))
    await _scan_task(strategy, closes)
...
@pytest.mark.asyncio
async def test_short_flips_to_long_on_long_zone() -> None:
    closes = tuple([100.0] * 38 + [50.0, 52.0])  # LONG zone
    strategy = strategy_with_position(
        ctx_for(closes, AsyncMock(return_value={"ok": True})), "SHORT"
    )
    await _scan_task(strategy, closes)

    calls = ctx_signals(strategy)
    assert [c[0] for c in calls] == ["CLOSE", "OPEN"]
    assert calls[0][1]["reason"] == "ZONE_FLIP"
    assert calls[1][1]["side"] == "LONG"
    assert len(strategy._positions) == 1


@pytest.mark.asyncio
async def test_long_flips_to_short_on_short_zone() -> None:
    closes = tuple([100.0] * 38 + [150.0, 148.0])  # SHORT zone
    strategy = strategy_with_position(
        ctx_for(closes, AsyncMock(return_value={"ok": True})), "LONG"
    )
    await _scan_task(strategy, closes)
...
async def test_long_held_in_between_zone() -> None:
    closes = tuple([100.0] * 40)  # close at mean -> BETWEEN
    strategy = strategy_with_position(
        ctx_for(closes, AsyncMock(return_value={"ok": True})), "LONG"
    )
    await _scan_task(strategy, closes)
...
@pytest.mark.asyncio
async def test_short_flushed_to_cash_in_between_zone() -> None:
    closes = tuple([100.0] * 40)  # between
    strategy = strategy_with_position(
        ctx_for(closes, AsyncMock(return_value={"ok": True})), "SHORT"
    )
    await _scan_task(strategy, closes)
    ...
    assert calls[0][1]["reason"] == "ZONE_BETWEEN_CASH"
    assert strategy._positions == {}


@pytest.mark.asyncio
async def test_same_side_zone_holds() -> None:
    closes = tuple([100.0] * 38 + [50.0, 52.0])  # LONG zone
    strategy = strategy_with_position(
        ctx_for(closes, AsyncMock(return_value={"ok": True})), "LONG"
    )
