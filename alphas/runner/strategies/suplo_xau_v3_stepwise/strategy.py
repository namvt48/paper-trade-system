"""Native runner implementation of the Supertrend XAU V3 Stepwise Dynamic Trailing strategy.

V3 integrates the Stepwise Dynamic Trailing r(P) algorithm based on 15m ATR:
  - Supertrend(3, 10) is computed on RAW 15m candles.
  - ATR(10) is computed on RAW 15m candles to define dynamic profit tiers.
  - Trailing take-profit is simulated MINUTE-BY-MINUTE on 1m candles:
      r(P) = None  if P < 0.5 * ATR (No early exit noise)
      r(P) = 35%   if 0.5 * ATR <= P < 0.75 * ATR
      r(P) = 30%   if 0.75 * ATR <= P < 1.0 * ATR
      r(P) = 25%   if 1.0 * ATR <= P < 1.5 * ATR
      r(P) = 20%   if P >= 1.5 * ATR (Lock 80% peak profit on breakout)
...
M1_MS = 60 * 1000
M15_MS = 15 * 60 * 1000
POSITION_NAMESPACE = uuid.UUID("a1b2c3d4-e5f6-7a8b-9c0d-1e2f3a4b5c6d")
...
def get_stepwise_retrace_r(peak_profit: float, atr_val: float) -> Optional[float]:
    """Calculate dynamic retracement r(P) according to ATR profit tiers."""
    if atr_val <= 0 or peak_profit < 0.5 * atr_val:
        return None
    elif peak_profit < 0.75 * atr_val:
        return 0.35
    elif peak_profit < 1.0 * atr_val:
        return 0.30
    elif peak_profit < 1.5 * atr_val:
        return 0.25
    else:
        return 0.20


def process_long_minute(
    minute_open: float,
    minute_high: float,
    minute_low: float,
    entry_price: float,
    ...
    """Causal minute-by-minute Long processing with Stepwise r(P)."""
    ...
    r_val = get_stepwise_retrace_r(peak_profit, atr_val)
    if r_val is None or peak_profit <= 0:
        return False, None, updated_best_high, active_trailing

    calculated_trailing = entry_price + peak_profit * (1.0 - r_val)
...
def process_short_minute(
    ...
    """Causal minute-by-minute Short processing with Stepwise r(P)."""
    if active_trailing is not None:
        if minute_open >= active_trailing:
            exit_price = minute_open * (1.0 + slippage_pct)
...
class SupertrendXauV3StepwiseRunnerStrategy(Strategy):
    """Supertrend(3,10) on 15m entries + 1m Stepwise r(P) Dynamic Trailing strategy."""
    ...
    async def _scan_1m(self, candle_open_ms: int) -> None:
        ...
        atr_val = float(position.get("latest_atr", 1.0))
        o, h, l = series.opens[-1], series.highs[-1], series.lows[-1]
        ...
        if side == "LONG":
            ...
            exited, exit_price, best_high, next_trailing = process_long_minute(
                o, h, l, entry, best, active, atr_val
            )
            ...
            active = position.get("active_trailing")
            active = float(active) if active is not None else None
            exited, exit_price, best_low, next_trailing = process_short_minute(
                o, h, l, entry, best, active, atr_val
            )
        ...
        if exited:
            await self._close_position(
                position, "STEPWISE_TRAILING_TP_r(P)", exit_price, candle_open_ms
            )
            return
    ...
    async def _open_if_allowed(
        ...
        position_id = str(
            uuid.uuid5(
                POSITION_NAMESPACE,
                f"{self.alpha_id}|{self.version}|stepwise|{candle_open_ms}|{side}",
            )
        )
        position = {
        ...
        metadata = {
            "strategy_type": "stepwise_dynamic_r_p",
    ...
    async def _close_position(
        ...
        await self.ctx.emit_signal(
            ...
            metadata=json.dumps(
                {
                    "source": "supertrend_xau_v3_stepwise",
