"""Native runner implementation of the two supertrend XAU V2 alpha presets.

Reference: docs/alpha_supertrend_trailing_5pct_v2.py and
docs/alpha_supertrend_trailing_10pct_v2.py.

V2 differs from the V1 suplo-xau family:
  - Supertrend(3, 10) is computed on RAW 15m candles (not M30 built from M15).
  - Trailing take-profit is simulated MINUTE-BY-MINUTE on 1m candles with a
    strict causal order per minute: (1) gap check at open, (2) check the
    trailing level from the PREVIOUS minute against low/high, (3) only if not
    exited, update the peak and compute the next trailing level, which takes
    effect from the following minute.
  - Exit prices include a reference slippage (0.0001).

Entry remains the Supertrend direction flip on 15m: a flip to downtrend (1)
opens SHORT, a flip to uptrend (-1) opens LONG.  After a trailing exit the
strategy stands in cash until the next 15m flip.
"""
...
def process_long_minute(
    ...
    """Port of the reference ``process_long_minute``.
...
def process_short_minute(
    ...
    """Port of the reference ``process_short_minute``.
...
class SupertrendXauV2RunnerStrategy(Strategy):
    """Supertrend(3,10) on 15m entries + 1m-simulated trailing TP.

    ``preset`` selects the trailing retrace: 5 -> 5%, 10 -> 10%.
    """
    ...
    def __init__(self, alpha_id: str, version: str, params: dict, ctx) -> None:
        ...
        if self.preset not in {5, 10}:
            raise ValueError("supertrend_xau_v2 preset must be 5 or 10")
    ...
    async def _scan_15m(self, candle_open_ms: int) -> None:
        ...
        await self._open_if_allowed(side, close, candle_open_ms)
    ...
    async def _scan_1m(self, candle_open_ms: int) -> None:
        ...
        if side == "LONG":
            ...
            exited, exit_price, best_high, next_trailing = process_long_minute(
                o, h, l, entry, best, active, self.retrace_pct
            )
