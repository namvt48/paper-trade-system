...
class Alpha1FixedEngine(BaseEngine):
    """Paper-trade engine implementing Alpha-1 (fixed position size).

    Signal logic (mirrors backtest_v5.py):
    - Trend state: acol (normalized SMA momentum) crossing ±threshold
    - Entry filter: close must deviate >2% from median(30) in the trend direction
    - Trailing SL: TRAIL_ATR_MULT × ATR(200), updated after SL/TP check each bar
    - TP: TRAIL_ATR_MULT × TP_RATIO × ATR(200)
    - CUT: counter-signal (acol flips or price crosses median filter)
    - REV: trend flip while in position, entry condition met → close + reopen

    Trend state is in-memory only; a restart resets all states to None.
    New entries are blocked until acol next crosses ±threshold after restart.
    """

    def __init__(self):
        super().__init__(settings)
        # position_id, side, entry, sl, tp, trail_distance, hse, lse
    ...
    def _process_symbol(self, row: dict) -> None:
        ...
        if pos is not None:
            if not trend_changed:
                ...
                # REV: trend flipped while in position — close and reopen if filter passes
    ...
    def _open_new_position(
        ...
        self.push_signal(
            ...
            metadata=json.dumps({
                ...
                "leverage": symbol_lev,
                "margin": round(settings.INVEST_PER_TRADE / symbol_lev, 2),
            }),
            timestamp=timestamp,
        )
