...
```
long_when : rank(cs_zscore(ts_zscore(close_t180, 60)) + cs_zscore(ts_zscore(volume_t180, 40))) > 0.9
short_when: rank(cs_zscore(ts_zscore(close_t180, 60)) + cs_zscore(ts_zscore(volume_t180, 40))) < 0.1
```

## Signal (score per symbol, then ranked cross-sectionally)

```
score = cs_zscore(ts_zscore(close_t180, 60)) + cs_zscore(ts_zscore(volume_t180, 40))
```
