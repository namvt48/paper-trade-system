...
from base.symbol_utils import get_binance_perp_symbols_by_volume_rank
...
class Alpha1Q3Engine(BaseEngine):
    """Alpha-1 Q3 Small Cap — rank 400-600 by 24h futures volume.

    Strategy params: trail 1.00-1.50x ATR, R:R 3.0:1, POC filter 4%, min hold 10 bars.
    Inherits all V5b logic (adaptive trail, Kelly sizing, time/min-hold guards).
    Wide parameters necessary due to thin books and higher volatility.
    """
    ...
    def _load_leverage_file(self) -> None:
        ...
        logger.info("[%s] Leverage map: %d symbols from %s", settings.ALPHA_ID, len(self._leverage_map), path)
    ...
    def _load_blacklist_file(self) -> None:
        ...
        logger.info("[%s] Blacklist: %d symbols from %s", settings.ALPHA_ID, len(loaded), path)

    def get_required_channels(self) -> list[str]:
        return [f"kline:{settings.TF}"]

    def _get_warmup_symbols(self) -> list[str]:
        syms = get_binance_perp_symbols_by_volume_rank(
            settings.VOLUME_RANK_START, settings.VOLUME_RANK_END
        )
        return [s for s in syms if not self._is_blacklisted(s)]
    ...
    async def _manage_positions(self) -> None:
        ...
        for symbol, snap in snapshots.items():
            ...
            self._manage_existing_position(
                symbol, self._open_positions[symbol],
                snap["close"], snap["high"], snap["low"], snap["current_time_ms"],
            )
    ...
    async def on_warmup_complete(self) -> None:
        ...
        for symbol, tf_map in self.symbol_data.items():
            ...
            trend = reconstruct_trend_state(
                list(sd.price_list), settings.SMA_LEN, settings.NORM_WINDOW, settings.THRESHOLD,
            )
            ...
            if not trend_changed:
                ...
                if bars >= settings.MIN_HOLD_BARS:
                    if side == "LONG" and acol < -threshold:
                        self._close_position(symbol, pos, close, "CUT", f"acol={acol:.3f}<-{threshold}"); cut = True
                    elif side == "LONG" and close < poc * (1 - poc_filter):
                        self._close_position(symbol, pos, close, "CUT", f"close<poc*(1-{poc_filter})"); cut = True
                ...
                if ce:
                    self._close_position(symbol, pos, close, "REV", f"trend→{'BULL' if trend else 'BEAR'}")
    ...
    def _manage_existing_position(
        self, symbol: str, pos: dict,
        close: float, high: float, low: float, current_time_ms: int = 0,
    ) -> None:
    ...
    def _open_new_position(
        self, symbol: str, trend: bool,
        close: float, high: float, low: float,
        atr: float, poc: float, acol: float, acol_prev: float,
        ...
        entry      = close
        sl = entry - trail_dist if side == "LONG" else entry + trail_dist
        ...
        self.push_signal(
            "OPEN",
            symbol=symbol, side=side, entry=entry, qty=qty, tp=tp, sl=sl,
            leverage=symbol_lev, position_id=position_id,
            exchange=settings.EXCHANGE, fee_pct=settings.FEE_PCT,
            metadata=json.dumps({
                "atr": round(atr, 6), "poc": round(poc, 6),
        ...
        logger.info(
            "[OPEN] %s %s @ %.6f sl=%.6f tp=%.6f trail=%.4f(%.3fx) size=%.2f lev=%dx equity=%.2f",
    ...
    def _close_position(self, symbol: str, pos: dict, exit_price: float, reason: str, detail: str = "", metadata: str | None = None) -> None:
        self.push_signal("CLOSE", position_id=pos["position_id"], exit_price=exit_price, reason=reason, metadata=metadata)
        ...
        wr = (sum(self._recent_results[-settings.KELLY_LOOKBACK:]) /
              min(len(self._recent_results), settings.KELLY_LOOKBACK) * 100) if self._recent_results else 0
        logger.info(
            "[CLOSE] %s reason=%s @ %.6f  net=%.2f  kelly_wr=%.0f%%(%d)  next_size=%.2f  equity=%.2f%s",
            symbol, reason, exit_price, net, wr, len(self._recent_results),
