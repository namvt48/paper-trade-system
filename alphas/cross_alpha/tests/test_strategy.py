...
from cross_alpha.strategy import build_funding_panel, resample_funding_to_native_cadence, select_positions
...
def test_vwap_reversion_signal_longs_breakout_shorts_breakdown():
    ...
    close = pd.DataFrame(
        {
            "BREAKOUTUSDT": [10.0] * 8 + [20.0, 25.0],  # stretched far above its own VWAP
            "BREAKDOWNUSDT": [10.0] * 8 + [5.0, 3.0],   # stretched far below its own VWAP
            "FLATUSDT": [10.0] * 10,
            "FLAT2USDT": [12.0] * 10,
        },
        index=idx,
...
def test_resample_funding_to_native_cadence_keeps_last_reading_per_bucket():
    ...
    idx = pd.Index([0, 2 * 3600 * 1000, 4 * 3600 * 1000, 6 * 3600 * 1000, _EIGHT_HOURS_MS], dtype="int64")
    funding = pd.DataFrame({"ALTUSDT": [0.0001, 0.0002, 0.0003, 0.0004, 0.0005]}, index=idx)
...
def test_carry_momentum_signal_longs_uptrend_cheap_funding_shorts_expensive():
    ...
    close = pd.DataFrame(
        {
            "CHEAPUSDT": [float(i) for i in range(1, 26)],   # strong uptrend, cheap funding
            "RICHUSDT": [float(i) for i in range(1, 26)],    # strong uptrend, expensive funding
            "FLATUSDT": [5.0] * 25,
            "FLAT2USDT": [6.0] * 25,
        },
        index=idx,
...
def test_ideal_amplitude_signal_longs_positive_amp_close_correlation():
    ...
    low = pd.DataFrame({"POSAMPUSDT": [100.0] * 9, "NEGAMPUSDT": [100.0] * 9}, index=idx)
...
def test_trend60cmf_has_its_own_137_symbol_whitelist():
    ...
    trend60cmf_symbols = [
        ...
        (alphas_root / "1d-trend60cmf" / "whitelist.txt").read_text().splitlines()
        if line.strip()
    ]
    shared_symbols = {
        line.strip() for line in
        (alphas_root / "1h-decay-close" / "whitelist.txt").read_text().splitlines()
        if line.strip()
    }
