...
class Alpha1V5b28PctReverseEngine(BaseEngine):
    """Paper-trade engine for Alpha-1 V5b with fixed 2.8% hold TP/SL (Reverse).
    ...
    - Fixed TP/SL during MIN_HOLD_BARS, then switch to calculated TP/SL and trailing
    ...
    def _fixed_tp_sl(self, side: str, entry: float) -> tuple[float, float]:
        pct = settings.INITIAL_FIXED_TP_SL_PCT
        if side == "LONG":
            return entry * (1 - pct), entry * (1 + pct)
        return entry * (1 + pct), entry * (1 - pct)

    def _activate_post_hold_tpsl(self, symbol: str, pos: dict) -> None:
        if pos.get("post_hold_tpsl_activated"):
            return

        side = pos["side"]
        target_sl = pos["post_hold_sl"]
        target_tp = pos["post_hold_tp"]
        if side == "LONG":
            new_sl = max(pos["sl"], target_sl)
        else:
            new_sl = min(pos["sl"], target_sl)

        pos["sl"] = new_sl
        pos["tp"] = target_tp
        pos["post_hold_tpsl_activated"] = True
        self.push_signal("MODIFY", position_id=pos["position_id"], sl=new_sl, tp=target_tp)
        logger.debug(
            "[MODIFY] %s activate post-hold tpsl sl=%.6f tp=%.6f",
            symbol, new_sl, target_tp,
        )

    def _check_candle_tpsl(self, symbol: str, pos: dict, high: float, low: float) -> bool:
        side = pos["side"]
        if side == "LONG":
            if low <= pos["sl"]:
    ...
    def _check_tick_tpsl(self, symbol: str, pos: dict, price: float, msg: dict) -> bool:
        side = pos["side"]
        if side == "LONG":
            if price <= pos["sl"]:
                self._close_position(
    ...
    def _manage_existing_position(
        ...
        During MIN_HOLD_BARS only the fixed entry TP/SL is active.
        After MIN_HOLD_BARS, switch to calculated TP/SL and continuous trailing SL.
        ...
        if current_time_ms and self._bars_held(pos, current_time_ms) < settings.MIN_HOLD_BARS:
            self._check_candle_tpsl(symbol, pos, high, low)
            return

        if current_time_ms:
            self._activate_post_hold_tpsl(symbol, pos)
    ...
    def on_price_alert_message(self, msg: dict) -> None:
        ...
        if self._bars_held(pos, current_time_ms) < settings.MIN_HOLD_BARS:
            self._check_tick_tpsl(symbol, pos, price, msg)
            return
    ...
    def _open_new_position(
        ...
        if not self.can_open_new_trades():
            return
        side       = "SHORT" if trend else "LONG"
        trail_dist = self._adaptive_trail(acol, atr)
        tp_dist    = trail_dist * settings.TP_RATIO
        entry      = close

        if side == "SHORT":
            post_hold_sl = entry + tp_dist
            post_hold_tp = entry - trail_dist
            hse, lse = entry, entry
        else:
            post_hold_sl = entry - tp_dist
            post_hold_tp = entry + trail_dist
            hse, lse = entry, entry
        sl, tp = self._fixed_tp_sl(side, entry)
        ...
        self._open_positions[symbol] = {
            ...
            "sl":                   sl,
            "tp":                   tp,
            "post_hold_sl":         post_hold_sl,
            "post_hold_tp":         post_hold_tp,
            "post_hold_tpsl_activated": False,
        ...
        self.push_signal(
            ...
            metadata=json.dumps({
                "atr":           round(atr, 6),
                "poc":           round(poc, 6),
                "initial_fixed_tp_sl_pct": settings.INITIAL_FIXED_TP_SL_PCT,
                "post_hold_sl":  round(post_hold_sl, 6),
                "post_hold_tp":  round(post_hold_tp, 6),
        ...
        logger.info(
            "[OPEN] %s %s @ %.6f fixed_sl=%.6f fixed_tp=%.6f post_sl=%.6f post_tp=%.6f trail=%.4f(%.3fx) size=%.2f lev=%dx equity=%.2f | acol=%.3f→%.3f x%+.3f poc=%.6f ratio=%.4f",
            side, symbol, entry, sl, tp, post_hold_sl, post_hold_tp, trail_dist, trail_dist / atr,
